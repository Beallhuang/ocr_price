...
def detect_s():
    ...
    source, weights, view_img, save_txt, imgsz, save_result = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.save_result
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_result:
                price = [[*xyxy, names[int(cls)]] for *xyxy, conf, cls in reversed(det)]
                price.sort()
                price = ''.join([i[-1] for i in price])
                with open(os.path.join(os.path.dirname(os.path.dirname(txt_path)), 'price.txt'), "a") as f:
                    f.write(str(p.stem) + '\t' + price + '\n')
